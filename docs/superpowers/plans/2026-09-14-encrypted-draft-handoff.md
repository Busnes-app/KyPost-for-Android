...
- [x] In `ComposeActivity`, snapshot/export the draft once, resolve the webmail URL first, then execute the decision. For `ENCRYPT_AND_SAVE`, construct `ClientEncryptedDraftSaver` with `AndroidVaultOpener`, the controller's account address, and `repository.saveClientEncryptedDraft`; open webmail only after the encrypted save succeeds, and retain the composer even on success. The relay acknowledgement does not prove ciphertext storage on older servers.
